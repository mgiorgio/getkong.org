...
Setup the "Blue" environment, running version 1 of the address service;
...
Now before deployment of version 2 of the address service we can set up the
"Green" environment.

```bash
# create an new Green upstream for address service v2
$ curl -X POST http://kong:8001/upstreams \
    --data "name=address.v2.service"

# add a targets to the upstream
...
Now requests with host header set to `address.mydomain.com` will now be proxied
by Kong to the new targets; 1/2 of the requests will go to
...
Using the ring-balancer the target weights can be adjusted very granular and
